...
def set_lifx_brightness(token, selector, brightness): # token parameter to access an accounts LIFX bulbs, selector parameter to specify which bulb to control, brightness paramater to specify the desired brightnesslevel
...
responseData = set_lifx_brightness(api_token, "all", brightness_level) #this executes the set_lifx_brightness to constructs and send a request over to the LIFX system with desiered .   
